...
| [PRE_DEPLOYMENT_CHECKLIST.md](PRE_DEPLOYMENT_CHECKLIST.md) | Generic pre-launch checklist for Vite + React + Supabase CustomApps consuming the Viewer bundle (custom SMTP, prod auth URLs, migration replay, RLS audit, hosting + env vars, bundle pinning decision, smoke test, error tracking, etc.). Copy into your repo and **extend with your CustomApp's own items** — the template is a starting point, not exhaustive. |
...
7. **For persistence beyond DemoApp's localStorage demo**: see the [Persistence Patterns](https://ebjornson.github.io/viewer-docs/persistence_patterns/) doc. localStorage's ~5MB quota can't hold user-uploaded `.glb` files; the doc covers IndexedDB / OPFS / Supabase / Firebase / S3 options behind a small storage abstraction so v0.1 can swap to a backend without restructuring. Includes a refined storage interface based on real CustomApp shape, a `useReducer` recommendation for App-side project state, the React 19 StrictMode blob-URL trap (with safe pattern), the cross-table RLS `SECURITY DEFINER` workaround, the versioned migration pattern, and a pointer to the `PRE_DEPLOYMENT_CHECKLIST.md` template above.

**Maintain your CustomApp's own non-obvious-invariants doc.** As you discover load-bearing things — pinned versions, RLS quirks, browser-specific gotchas, dev/prod parity issues — capture them in a project-memory file (CLAUDE.md if you're using Claude Code; any equivalent if not). Real CustomApp builds reliably surface ~10-20 of these per phase, and the doc is what keeps a future-you (or a teammate) from re-discovering them the hard way. The publicly-rendered Viewer docs themselves are roughly that pattern at the platform level — every invariant in [Capture & Replay](https://ebjornson.github.io/viewer-docs/capture_and_replay/), the [Persistence Patterns](https://ebjornson.github.io/viewer-docs/persistence_patterns/) callouts, and the bridge's "Why this exists" docstring all started as load-bearing observations someone wrote down so the next person didn't have to rediscover.
