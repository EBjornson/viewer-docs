...
This supersedes the Viewer's static import of [`materialManifest`](../src/config/materialManifest.js). The App constructs and supplies it on every render.
...
Pull the filename-pattern logic and manifest-record construction out of [`scripts/generate-material-manifest.mjs`](../scripts/generate-material-manifest.mjs) into a runtime-callable module, e.g. `src/utils/materialPackageParser.js`.
...
Update App-side replay (in [`DemoApp.jsx`](../src/DemoApp/DemoApp.jsx) and any helpers under [`src/DemoApp/`](../src/DemoApp/)) to:
...
The static import of [`materialManifest`](../src/config/materialManifest.js) inside the Viewer is removed in this phase; the manifest still exists in the codebase as the platform catalog source, just consumed by the App now, not the Viewer.
...
This plan keeps the curated catalog in-repo (Path A). When the curated catalog grows past what's comfortable to ship in a deploy, or when a non-engineer needs to add curated materials without engineering involvement, the platform catalog migrates to the same cloud storage as workspace materials, served through the same API with `source: 'platform'`. The App-side resolver doesn't change. The static [`materialManifest.js`](../src/config/materialManifest.js) and [`generate-material-manifest.mjs`](../scripts/generate-material-manifest.mjs) are retired or repurposed as a CLI for seeding the platform bucket.
