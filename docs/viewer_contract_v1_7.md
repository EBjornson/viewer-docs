...
**`presentationSyncKey`:** An optional monotonically-increasing counter the App increments to signal **"App selection changed"** (a section, view, or other selection event the Viewer should react to). The Viewer interprets a changed value at two layers:
...
**Bump on every section/view selection change** — captured or not. The Viewer's two-layer interpretation handles both cases correctly. There is no need (and no harm) in bumping when the App has no capture to replay; the re-sync layer simply skips when `input.presentation` is `undefined`.
