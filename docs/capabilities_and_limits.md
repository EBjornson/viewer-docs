...
**Overhead space-tile click (`SpaceTileClickNav`):** When in overhead view, clicking a space tile navigates the camera to that interior space via pathNav **and also applies the Interior view capture** — including its visibility assignments and presentation settings (lighting, environment, etc.). This is the same state that is applied when pressing the Interior View button. If no Interior view capture has been authored, the click still navigates the camera but leaves presentation state unchanged.
