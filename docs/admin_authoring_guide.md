...
A section capture stores the **full presentation snapshot embedded** alongside camera and visibility, but the section tab itself exposes only camera/visibility controls — adjust lighting via the pMode tab and capture it there separately, or use the App-rendered pMode pills in the DemoApp header to load App-stored snapshots as starting points (always live in admin mode; hidden in user mode by default and revealed by DemoApp's opt-in Visual Override toggle).
...
2. **Capture Sections.** Section captures are the primary "stored moments" — they hold pose, cameraMode, embedded presentation snapshot, visibility, and User Visibility flags. For each section: click the section tab in the App, navigate (use the admin-only Quickviews row at the top of the AuthoringPanel's Section tab as quick starting points if useful — Quickview clicks set the capture's `cameraMode` automatically; further pan/zoom/orbit refines the pose without changing the mode), load a pMode pill in the App header to apply the right starting lighting, set User Visibility, hide any presentation-context geometry (e.g. roof for an overhead view), and click **Section Capture**. Optionless sections serve as "view-like" stored moments — author them the same way.
